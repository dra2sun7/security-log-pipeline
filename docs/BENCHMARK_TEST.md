...
| **데이터 처리 완료 속도** | **연산 정체 장시간 지속 (병목)** | **초고속 연산 종료 후 자원 회수 ⚡** | **동일 자원 대비 데이터 파이프라인 처리량(TPS) 폭발** |

---

### 🔍 2. 실측 데이터 기반 성능 역설(Performance Paradox) 아키텍처 해석

1. **최고점 CPU(Peak CPU)의 유사 현상 분석**:
   - 아키텍처 전 구간을 JSON으로 전환하여 Grok 정규식이라는 무거운 연산 비용을 완벽히 제거했음에도 불구하고 Logstash의 CPU가 `336.15%`까지 포화되는 현상을 관측함.
   - 이는 연산 단위 자체가 가벼워짐에 따라, Logstash 내부의 멀티스레드 디코딩 엔진이 쉬지 않고 **Kafka 큐에 쌓인 10만 건의 메시지를 지연 없이 최대 스피드로 흡수·가공(Throttling Up)**하면서 발생한 '자원 집약적 고속 처리' 현상임.
