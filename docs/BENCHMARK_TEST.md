...
## ⚡ [시나리오 4] Grok 정규식 배제를 통한 수집 가공(ETL) CPU 부하 최적화 테스트
- **테스트 일시**: 2026-07-06
- **테스트 목적**: 파이프라인 레이어에서 CPU 연산 비용이 가장 큰 Grok 정규식 패턴을 제거하고, 엔드투엔드(Nginx-Filebeat-Logstash) 구간을 JSON 아키텍처로 통일하여 대량 부하 상황에서의 가공 처리 효율성 극대화 실증 검증

### 📍 1. 10만 건 대량 부하(댐 가두기) 실측 지표 대조

| 측정 지표 (100,000건 방류 시) | 시나리오 3 (Grok 정규식 변환) 🚨 | 시나리오 4 (JSON 다이렉트 디코딩) 🎯 | 실질 인프라 개선 성과 및 해석 |
| :--- | :--- | :--- | :--- |
| **Nginx 자원 소모** | **`0.00%`** / `5.391 MiB` | **`0.00%`** / `4.129 MiB` | **웹 서버 오버헤드 0% 유지 (원천 소스 안정성 보존)** |
| **Logstash Peak CPU** | **`341.49%`** | **`336.15%`** (유사 수준 포화) | **연산 경량화로 인한 스레드 처리 속도 한계치 가동** |
| **Logstash 메모리 점유** | **`442.1 MiB`** | **`566.0 MiB`** | **문자열 버퍼 및 JSON 디코딩 힙 메모리 정상 가동** |
| **10만 건 소화 완료 시간** | **`31.4 초`** (연산 정체 지속) | **`5.2 초`** (초고속 연산 종료) ⚡ | **데이터 자원 회수 및 처리 속도 약 6배 단축** |

---

### 🔍 2. 실측 데이터 기반 성능 역설(Performance Paradox) 아키텍처 해석

1. **최고점 CPU(Peak CPU)의 유사 현상 및 시간 단축 분석**:
   - 아키텍처 전 구간을 JSON으로 전환하여 Grok 정규식이라는 무거운 연산 비용을 완벽히 제거했음에도 불구하고 Logstash의 CPU가 `336.15%`까지 포화되는 현상을 관측함.
   - 단일 시점의 CPU 소모량은 유사했으나, 10만 건의 메시지를 완전히 소화하고 자원이 평온(0% 대)해지는 데 걸린 총 시간이 **`31.4초`에서 `5.2초`로 무려 6배(약 26초) 가량 대폭 단축**됨을 실측함.
   - 이는 연산 단위 자체가 가벼워짐에 따라, Logstash 내부의 멀티스레드 디코딩 엔진이 쉬지 않고 Kafka 큐에 쌓인 10만 건의 메시지를 지연 없이 최대 스피드로 흡수·가공(Throttling Up)하면서 짧은 시간 동안 자원을 집약적으로 활용했음을 증명함.
2. **원천 웹 서버(Nginx)의 완벽한 리소스 보존**:
   - 10만 건의 고밀도 트래픽을 처리하는 과정에서도 Nginx는 CPU **`0.00%`**, 메모리 **`4.129 MiB`**라는 극단의 가벼움을 유지함. 수집 파이프라인의 고도화를 위해 최전방 원천 소스 설정을 변경하더라도 본질적인 웹 서비스 가용성에는 단 1%의 악영향도 주지 않음을 정량 실측으로 증명함.

---

### 🎯 3. 최종 성능 최적화 결론

* **진정한 의미의 파이프라인 처리 속도(Throughput) 확보**:
  - 소프트웨어적인 정규식 비용을 제거함으로써, 시스템이 낼 수 있는 최대 효율로 자원을 100% 활용해 데이터를 빠르게 소화해 내는 아키텍처적 고도화를 달성함.
* **엔드투엔드 데이터 규격 표준화의 가치 실증**:
  - 데이터 발생지(Nginx)부터 수집기(Filebeat), 버퍼(Kafka), 가공기(Logstash)까지 데이터 포맷을 **JSON 구조로 단일화(Streamlining)**하는 표준화 설계가 왜 실무 대용량 파이프라인 환경에서 비용 절감과 성능 확장(Scale-out)의 필수 정석인지 직접 데이터로 입증 완료함.
