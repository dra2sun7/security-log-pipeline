...
graph TB
    ...
    subgraph Web_Layer [웹 서비스 계층]
        Nginx["🛡️ Nginx Web Server<br/>(Generate JSON access.log)"]
    end
    ...
    subgraph Processing_Layer [가공 및 저장 계층]
        Logstash["⚙️ Logstash<br/>(JSON Decoding & Masking)"]
...
### 🧩 컴포넌트별 핵심 역할
* **Nginx**: 실제 상용 환경의 웹 서비스를 대변하며 보안 분석의 대상이 되는 원본 로그를 생성합니다. 아키텍처 고도화를 위해 기존 텍스트 형태의 로그 대신 처음부터 **구조화된 JSON 포맷**으로 로그를 출력하도록 개선했습니다.
...
* **Logstash**: 데이터 전 구간의 포맷을 JSON으로 일원화함에 따라 무거운 문자열 정규식(Grok) 연산 비용을 완전히 제거했습니다. 카프카에서 데이터를 가져와 가벼운 **JSON 다이렉트 디코딩**만 수행하며, 내부 자산 및 개인정보 보호를 위한 **IP 주소 뒷자리 마스킹 처리(`172.18.0.xxx`)**를 담당하는 ETL 엔진입니다.
...
4. **JSON 포맷 단일화를 통한 가공 연산 최적화 및 다각도 소거법 검증 (시나리오 4)**
   - 무거운 문자열 정규식(Grok) 대신 데이터 전 구간 규격을 JSON 하나로 통일하는 아키텍처 최적화 진행.
   - 단순히 표면적인 전체 처리 시간에 안주하지 않고, 후방 저장소를 차단하는 **원인 소거법(Null Output) 실험**과 **실시간 타임스탬프 로그 추적**을 통해 인프라 내부 동작을 다각도로 정밀 교차 검증함.
   - 실험 결과, 어떤 방식을 쓰든 시스템 초기 구동 및 카프카 조인에 고정적인 준비 대기 시간(약 17초)이 공통적으로 기저에 상존함을 규명함.
   - 이 초기 기동 오버헤드를 제외한 **10만 건 데이터의 '순수 가공 연산 시간'을 산출했을 때, 기존 Grok 방식(6초) 대비 JSON 포맷 통일 방식(3초)이 가공 성능을 정확히 2배(200%) 개선**했음을 정량적으로 증명함. 더불어 전체 파이프라인의 종단 처리 속도를 제어하는 병목 지점(Elasticsearch 디스크 쓰기 버퍼 포화)까지 과학적으로 파악해 내는 성과를 달성함.
